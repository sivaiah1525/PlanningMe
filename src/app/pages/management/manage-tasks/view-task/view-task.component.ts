...
export class ViewTaskComponent implements OnInit {
  ...
  showViewDetails() {
    this.loader=true
    this.userService.FindTaskById(this.data).subscribe((res: any) => {
      console.log("ressss",res)
      this.taskData = res;
      this.loader=false
      if(res.hasStep==true&&res.stepsCount!=0){
             this.userService.FindTaskSteps(this.data).subscribe((result)=>{
              if(result){
...
Updatestepstatus(id:any){
  ...
  FindUserbyTaskId() {
    this.userService.FindUserByTaskId(this.data).subscribe(res => {
      console.log(res)
  ...
  findContactLinked(count:any) {
    if (count > 0) {
      this.userService.FindContactsByTaskId(this.data).subscribe(res => {
        console.log(res)
  ...
  findInitiativeLinked(count:any) {
    if (count > 0) {
      this.userService.FindInitiativesByTaskId(this.data).subscribe(res => {
        console.log(res)
  ...
  openEditdEventDialog(id:number) {
  ...
  getStatusColorClass(status: string) {
    ...
    }   else if (status == 'In Progress'||status == 'In_Progress'||status == 'in Progress') {
