...
export class CreateTaskComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.screenType=='Create-task'||this.screenType=='Create-task-initiatives'||this.screenType=='Create-subTask'||this.screenType=='addsubtask'){
      this.gettags();
    }
...
filterOptionsforExistingPipeline(value: string): void {
  ...
  onSubmit() {
    ...
    formData.append('SubTaskViewModels','');
    if (this?.value == 1) {
    ...
    console.log(this.form?.value.Addtages)
    if (this.form?.value.Addtages.length != 0) {
      console.log(this.form?.value.Addtages)
      this.form?.value.Addtages.forEach((e:any,index:any) => {
        console.log(index)
